...
def initialize_database():
    ...
    # Create VotingSessions table for managing voting sessions
    cursor.execute('''
    CREATE TABLE IF NOT EXISTS VotingSessions (
        game_id TEXT PRIMARY KEY,
        is_anonymous INTEGER DEFAULT 0,
        summary_message_id INTEGER,
        permissions_message_id INTEGER,
        FOREIGN KEY (game_id) REFERENCES Games(game_id)
    )
    ''')

    # Create VoterPermissions table for tracking who can vote and be voted
    cursor.execute('''
    CREATE TABLE IF NOT EXISTS VoterPermissions (
        game_id TEXT,
        user_id INTEGER,
        can_vote INTEGER DEFAULT 1,
        can_be_voted INTEGER DEFAULT 1,
    ...
    # Create Votes table for tracking actual votes
    cursor.execute('''
    CREATE TABLE IF NOT EXISTS Votes (
        ...
        PRIMARY KEY (game_id, voter_id, target_id)
    )
    ''')
